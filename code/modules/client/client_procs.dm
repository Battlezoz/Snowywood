...
/client/proc/show_character_previews(mutable_appearance/MA, map_key = "character_preview_map")
	var/pos = 0
	...
	screen += background
	background.screen_loc = "[map_key]:0,0 to 3,3"
	...
	for(var/D in GLOB.cardinals)
		...
		switch(pos)
			if(1)
				O.screen_loc = "[map_key]:2,2"
			if(2)
				O.screen_loc = "[map_key]:1,2"
			if(3)
				O.screen_loc = "[map_key]:1,1"
			if(4)
				O.screen_loc = "[map_key]:2,1"
